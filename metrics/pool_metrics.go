...
func NewPoolMetrics() *PoolMetrics {
	m := &PoolMetrics{
		...
		ContainerAnnounceUsedSpace: prometheus.NewHistogram(prometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: poolSubsystem,
			Name:      "container_announce_used_space",
			Help:      "Container announce used space request handling time",
		}),
		AnnounceIntermediateTrust: prometheus.NewHistogram(prometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: poolSubsystem,
			Name:      "announce_intermediate_trust",
			Help:      "Announce intermediate trust request handling time",
		}),
		AnnounceLocalTrust: prometheus.NewHistogram(prometheus.HistogramOpts{
			Namespace: namespace,
			Subsystem: poolSubsystem,
			Name:      "announce_local_trust",
			Help:      "Announce local trust request handling time",
		}),
...
func (m *PoolMetrics) OperationCallback(nodeKey []byte, endpoint string, method stat.Method, duration time.Duration, err error) {
	...
	case stat.MethodContainerAnnounceUsedSpace:
		m.ContainerAnnounceUsedSpace.Observe(duration.Seconds())
	case stat.MethodAnnounceIntermediateTrust:
		m.AnnounceIntermediateTrust.Observe(duration.Seconds())
	case stat.MethodAnnounceLocalTrust:
