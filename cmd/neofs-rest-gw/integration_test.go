...
func restObjectDeleteSessionV2(ctx context.Context, t *testing.T, p *pool.Pool, owner *user.ID, cnrID cid.ID, signer, signerForToken user.Signer) {
	...
	tokenRequest := apiserver.SessionTokenV2Request{
		...
		Targets:  []apiserver.TokenTarget{{Owner: gateUserID.String()}},
	}
...
func v2AuthSessionToken(ctx context.Context, t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		request := apiserver.SessionTokenV2Request{
			...
			Expiration: 48,
			Owner:      ownerID.String(),
			Targets:    []apiserver.TokenTarget{{Owner: targetID.String()}},
		...
		t.Run("stitch session token", func(t *testing.T) {
			signedData, err := base64.StdEncoding.DecodeString(response.SignedData)
			require.NoError(t, err)
			...
			require.True(t, sessionToken.Exp().After(time.Now().Add(47*time.Hour+59*time.Minute)))
	...
	t.Run("different contexts with one container", func(t *testing.T) {
		...
		httpClient := defaultHTTPClient()
		makeV2AuthSessionTokenRequest(ctx, t, request, httpClient, http.StatusBadRequest, "two different contexts have one container")
	})
}
