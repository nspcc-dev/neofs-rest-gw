...
func (a *RestAPI) V2SearchObjects(ctx echo.Context, containerID apiserver.ContainerId, params apiserver.V2SearchObjectsParams) error {
	...
	slices.SortFunc(resSearch, func(a, b client.SearchResultItem) int {
		timestampA, _ := strconv.ParseInt(a.Attributes[indexes.Timestamp], 10, 64)
		timestampB, _ := strconv.ParseInt(b.Attributes[indexes.Timestamp], 10, 64)
		return cmp.Compare(timestampA, timestampB)
	})

	var objects = make([]apiserver.ObjectBaseInfo, len(resSearch))
