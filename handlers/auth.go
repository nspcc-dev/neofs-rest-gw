...
func (a *RestAPI) V2AuthSessionToken(ctx echo.Context) error {
	...
	for _, target := range apiParams.Targets {
		...
		if target.Owner != "" {
			...
			parsed, err := url.Parse(target.NnsName)
			if err != nil {
				return ctx.JSON(http.StatusBadRequest, a.logAndGetErrorResponse("invalid target nns name", err, log))
			}

			if parsed.Path != "" {
				return ctx.JSON(http.StatusBadRequest, a.logAndGetErrorResponse("nns name shouldn't have any path", err, log))
			}
			if parsed.Scheme != "" {
				return ctx.JSON(http.StatusBadRequest, a.logAndGetErrorResponse("nns name shouldn't have scheme (http, https, etc)", err, log))
			}
			if len(parsed.Query()) > 0 {
				return ctx.JSON(http.StatusBadRequest, a.logAndGetErrorResponse("nns name shouldn't have query parameters", err, log))
			}
	...
	for _, apiTokenContext := range apiParams.Contexts {
		...
		if apiTokenContext.ContainerID != "" {
			...
			if _, ok := uniqueCnrIDs[cnrID]; ok {
				return ctx.JSON(http.StatusBadRequest, a.logAndGetErrorResponse("two different contexts have one container", fmt.Errorf("containerID: %s", cnrID.String()), log))
			}
...
func (a *RestAPI) V2CompleteAuthSessionToken(ctx echo.Context) error {
	...
	if scheme == neofscrypto.N3 {
		sessionToken.AttachSignature(neofscrypto.NewN3Signature(signatureKey, signatureValue))
	} else {
...
func (a *RestAPI) UnsignedBearerToken(ctx echo.Context, params apiserver.UnsignedBearerTokenParams) error {
	...
	if params.XBearerOwner != nil {
		if err := tokenOwner.DecodeString(*params.XBearerOwner); err != nil {
