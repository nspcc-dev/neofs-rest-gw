...
const (
	// hkdfInfo must match the value used by neofs-s3-gw, otherwise S3
	// gateways won't be able to decrypt secrets produced here.
	hkdfInfo       = "neofs-s3-gw"
	hkdfSaltLength = 16
)

// Encrypt encrypts data with a key derived from the owner private key and
...
// Decrypt decrypts data with a key derived from the owner private key and
// the sender public key.
